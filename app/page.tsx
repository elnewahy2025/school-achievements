...
import {
  ...
  X, ChevronLeft, ChevronRight, Download, Phone, MapPin, Star, Loader2
} from 'lucide-react';
...
export default function GalleryPage() {
  ...
  useEffect(() => {
    ...
    // Fetch full department list from API for the filter dropdown
    fetch('/api/departments')
      .then((r) => r.json())
      .then((data) => setDepartments(data.map((d: any) => d.name).sort()))
      .catch(() => {});
  }, []);
  ...
  const imageFiles = lightbox
    ? lightbox.achievement.files.filter((f) => f.file_type === 'image')
    : [];
  const pdfFiles = lightbox
    ? lightbox.achievement.files.filter((f) => f.file_type === 'pdf')
    : [];
    ...
    <div className="min-h-screen">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
          <div className="text-center animate-fade-in">
            ...
            <p className="text-xl sm:text-2xl text-white/90 font-medium mb-6 drop-shadow">
              {settings.tagline || 'Celebrating Academic & Extracurricular Excellence'}
            </p>
              ...
              <div className="flex flex-wrap justify-center gap-4 text-white/80 text-sm">
                {settings.manager_name && (
                  <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full">
                    ...
                    <span>Managed by {settings.manager_name}</span>
                  </div>
                )}
                {settings.phone && (
                  <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mb-6">
        <p className="text-gray-400 text-sm">
          {loading ? 'Loading...' : `${achievements.length} achievement${achievements.length !== 1 ? 's' : ''} found`}
        </p>
      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400">
              {searchQuery || selectedDept || selectedTeacher
            ...
            {achievements.map((achievement, index) => {
              ...
              return (
                <div
                  ...
                  className={`bg-dark-800 rounded-2xl overflow-hidden border border-dark-600 card-hover animate-slide-up ${
                ...
                >
                  ...
                  <div className="relative h-48 overflow-hidden">
                    {images.length > 0 ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={`/uploads/${achievement.id}/${images[0].stored_name}`}
                    ...
                    <div className="absolute top-3 left-3">
                      <div className="dept-badge px-3 py-1 rounded-full text-xs font-bold text-white shadow-lg">
                  ...
                  <div className="p-5">
                    ...
                    {achievement.description && (
                      <p className="text-gray-400 text-sm line-clamp-2 mb-3">{achievement.description}</p>
                    )}
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <div className="flex items-center gap-1.5">
                        <User className="w-3 h-3" />
                        <span>{achievement.teacher_name}</span>
                      </div>
                      {achievement.event_date && (
                        <div className="flex items-center gap-1.5">
                          <Calendar className="w-3 h-3" />
                          <span>{new Date(achievement.event_date).toLocaleDateString()}</span>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {lightbox && (
        <div
          className="fixed inset-0 bg-black/95 backdrop-blur-md z-50 flex flex-col animate-fade-in"
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-sm text-gray-400">
                <span className="flex items-center gap-1">
                  <User className="w-3 h-3" />
                  {lightbox.achievement.teacher_name}
                </span>
                <span className="dept-badge px-2 py-0.5 rounded-full text-xs text-white">
                  {lightbox.achievement.department}
                </span>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
            ...
            {imageFiles.length > 0 ? (
              <div className="flex-1 flex items-center justify-center relative p-4">
                ...
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={`/uploads/${lightbox.achievement.id}/${imageFiles[lightbox.imageIndex]?.stored_name}`}
                  alt={lightbox.achievement.title}
                ...
                {imageFiles.length > 1 && (
                  <button
                    onClick={(e) => { e.stopPropagation(); navigateLightbox(1); }}
                    disabled={lightbox.imageIndex === imageFiles.length - 1}
                    ...
                    {imageFiles.map((_, i) => (
                      <button
                        ...
                        className={`w-2 h-2 rounded-full transition-all ${
                          i === lightbox.imageIndex ? 'bg-kahoot-yellow w-6' : 'bg-white/40'
                        }`}
                      />
                    ))}
                  </div>
            ...
            {/* Details Panel */}
            <div className="w-full lg:w-96 bg-dark-800 border-l border-dark-600 p-6 overflow-y-auto">
              ...
              {(lightbox.achievement.event_date && formatDate(lightbox.achievement.event_date)) && (
                <div className="mb-6">
                  <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Event Date</h3>
                  <p className="text-gray-300 text-sm flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-kahoot-yellow" />
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {pdfFiles.map((file) => (
                      <a
                        key={file.id}
                        href={`/uploads/${lightbox.achievement.id}/${file.stored_name}`}
                        target="_blank"
                      ...
                      >
                        <div className="w-10 h-10 rounded-lg bg-kahoot-red/20 flex items-center justify-center">
                          <FileText className="w-5 h-5 text-kahoot-red" />
                        </div>
              ...
              {imageFiles.length > 1 && (
                <div className="mt-6 pt-6 border-t border-dark-600">
                  <p className="text-xs text-gray-500 text-center">
                    Use ← → arrows or click dots to browse {imageFiles.length} images
                  </p>
                </div>
              )}
            </div>
      ...
      <footer className="bg-dark-800/50 border-t border-dark-600 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
          <p className="text-gray-500 text-sm">
            © {new Date().getFullYear()} {settings.school_name || 'School Achievements'}. Built with{' '}
            <span className="text-kahoot-red">♥</span> and Kahoot-inspired vibes.
          </p>
        </div>
      </footer>
    </div>
