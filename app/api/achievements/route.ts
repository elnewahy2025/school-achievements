...
export async function GET(request: NextRequest) {
  try {
    ...
    if (account.is_admin) {
      achievements = db.prepare('SELECT * FROM achievements ORDER BY created_at DESC').all() as Achievement[];
    } else {
      achievements = db.prepare(
        'SELECT * FROM achievements WHERE teacher_id = ? OR department = ? ORDER BY created_at DESC'
      ).all(account.id, account.department) as Achievement[];
    }

    // Attach files to each achievement
    const stmt = db.prepare('SELECT * FROM files WHERE achievement_id = ?');
    achievements = achievements.map((a) => ({
      ...a,
      files: stmt.all(a.id) as any[],
    }));
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO achievements (title, description, department, teacher_id, teacher_name, event_date) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(title, description || '', department, account.id, account.full_name, dateValue);
    ...
    return NextResponse.json({
      ...
      teacher_name: account.full_name,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!account.is_admin && achievement.teacher_id !== account.id) {
      return NextResponse.json({ error: 'Cannot edit achievements by other teachers' }, { status: 403 });
    }
    ...
    db.prepare(
      'UPDATE achievements SET title = ?, description = ?, department = ?, event_date = ? WHERE id = ?'
    ).run(
