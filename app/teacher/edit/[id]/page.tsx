...
import { Save, X, FileText, Image as ImageIcon, Upload, Loader2, Trash2, Tag } from 'lucide-react';
...
export default function EditAchievement() {
  ...
  const fetchAchievement = useCallback(async () => {
    ...
    try {
      ...
      if (a) { setForm({ title: a.title, description: a.description, department: a.department, event_date: a.event_date || '', categories: a.categories || '' }); setExistingFiles(a.files); }
    ...
    <div className="min-h-screen pb-16">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div><label className="block text-sm font-medium text-gray-300 mb-2">{t('form.title')}</label><input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required className="w-full px-4 py-3 rounded-xl bg-dark-700 border border-dark-600 text-white placeholder-gray-400 text-sm" /></div>
          <div><label className="block text-sm font-medium text-gray-300 mb-2">{t('form.desc')}</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={4} className="w-full px-4 py-3 rounded-xl bg-dark-700 border border-dark-600 text-white placeholder-gray-400 text-sm resize-none" /></div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-300 mb-2">{t('form.dept')}</label><select value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })} required className="w-full px-4 py-3 rounded-xl bg-dark-700 border border-dark-600 text-white text-sm appearance-none">{departments.map((d: any) => (<option key={d.id} value={d.name}>{d.name}</option>))}</select></div>
            <div><label className="block text-sm font-medium text-gray-300 mb-2">{t('form.eventDate')}</label><input type="date" value={form.event_date} onChange={(e) => setForm({ ...form, event_date: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-dark-700 border border-dark-600 text-white text-sm" /></div>
          </div>
          {/* Categories */}
          <div>
            ...
            <input type="text" value={form.categories} onChange={(e) => setForm({ ...form, categories: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-dark-700 border border-dark-600 text-white placeholder-gray-400 text-sm" placeholder={t('form.categoriesPlaceholder')} />
            <p className="text-xs text-gray-500 mt-1">{t('form.categoriesHint')}</p>
            {form.categories && (<div className="flex flex-wrap gap-2 mt-2">{form.categories.split(',').filter(Boolean).map((c, i) => (<span key={i} className="px-2 py-0.5 rounded-full text-xs bg-kahoot-purple/20 text-kahoot-purple border border-kahoot-purple/30">{c.trim()}</span>))}</div>)}
          </div>
              ...
              <div className="space-y-2">
                {existingFiles.map((file) => (
                  <div key={file.id} className="flex items-center gap-3 p-3 rounded-xl bg-dark-700 border border-dark-600 group">
                    {file.file_type === 'image' ? (<img src={`/uploads/${achievementId}/${file.stored_name}`} alt={file.original_name} className="w-10 h-10 rounded-lg object-cover" />) : (<div className="w-10 h-10 rounded-lg bg-kahoot-red/20 flex items-center justify-center"><FileText className="w-5 h-5 text-kahoot-red" /></div>)}
                    ...
                    <button type="button" onClick={() => setDeleteFileModal({ open: true, file })} className="w-8 h-8 rounded-lg bg-dark-600 hover:bg-kahoot-red/20 flex items-center justify-center opacity-50 group-hover:opacity-100 transition-all shrink-0"><Trash2 className="w-4 h-4 text-kahoot-red" /></button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-all cursor-pointer ${dragActive ? 'border-kahoot-purple bg-kahoot-purple/10' : 'border-dark-600 hover:border-dark-500'}`}
              onDragOver={(e) => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} onDrop={(e) => { e.preventDefault(); setDragActive(false); handleFiles(e.dataTransfer.files); }} onClick={() => document.getElementById('file-input-edit')?.click()}>
              ...
              <input id="file-input-edit" type="file" multiple accept=".jpg,.jpeg,.png,.webp,.gif,.pdf" className="hidden" onChange={(e) => e.target.files && handleFiles(e.target.files)} />
            </div>
            {newFiles.length > 0 && (<div className="mt-3 space-y-2">{newFiles.map((file, i) => (
