...
export default function EditAchievement() {
  ...
  const handleDeleteFile = async () => {
    ...
    try {
      const res = await fetch(`/api/achievements/${achievementId}/files/${deleteFileModal.file.id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/achievements', {
        ...
        body: JSON.stringify({ id: parseInt(achievementId), ...form }),
      });
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error);
    ...
    <div className="min-h-screen pb-16">
      <div className="bg-gradient-to-r from-kahoot-purple/20 to-kahoot-blue/20 border-b border-dark-600">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          <h1 className="text-3xl font-black text-white">Edit Achievement</h1>
          <p className="text-gray-400 mt-2">Update your achievement details</p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {existingFiles.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Current Files</label>
                ...
                {existingFiles.map((file) => (
                  <div key={file.id} className="flex items-center gap-3 p-3 rounded-xl bg-dark-700 border border-dark-600 group">
                    {file.file_type === 'image' ? (
                      ...
                      <img
                        src={`/uploads/${achievementId}/${file.stored_name}`}
                    ...
                    <button
                      type="button"
                      onClick={() => setDeleteFileModal({ open: true, file })}
          ...
          {/* Upload New Files */}
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Add More Files</label>
            <div
            ...
            >
              ...
              <p className="text-gray-300 text-sm">
                Drag & drop or <span className="text-kahoot-purple">browse</span>
              </p>
              <p className="text-xs text-gray-500 mt-1">Images up to 10MB • PDFs up to 20MB</p>
              <input
                id="file-input-edit"
                type="file"
                multiple
      ...
      <DeleteModal
        isOpen={deleteFileModal.open}
        title="Delete File"
        message={`Are you sure you want to delete "${deleteFileModal.file?.original_name}"? This action cannot be undone.`}
        onConfirm={handleDeleteFile}
        onCancel={() => setDeleteFileModal({ open: false, file: null })}
